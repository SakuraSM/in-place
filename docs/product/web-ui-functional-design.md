...
| 总览 | `/overview` | 跨位置搜索和组合筛选全部库存 | `apps/web/src/features/inventory/pages/SearchPage.tsx` |
...
首页负责“今天要看和要做什么”，总览负责“查找某个对象”，空间位置负责“它在哪里”。三者职责不同，不应合并成同一种列表页。
...
- 当前“地图内资产”和“已定位总值”随筛选变化；“已标注位置”和“未定位资产”是全量统计。调整统计逻辑或文案时必须保持口径明确，并同步测试。
...
- 地图桌面端为“画布 + 详情侧栏”，移动端纵向排列统计、筛选、画布和详情。
...
| 优先级 | 风险 | 影响 | 建议方向 |
| --- | --- | --- | --- |
| P1 | 位置树和地图只改变查询参数，滚动复位目前只监听路径 | 在位置树向下滚动后切换地图，可能直接落到地图中段 | 视图切换时显式复位 `data-scroll-root`，并补回归测试 |
| P1 | 移动端统计和完整筛选占据一屏以上 | 用户进入页面后首屏看不到地图 | 压缩统计，筛选改为抽屉或可折叠区域 |
| P1 | 地图统计混合全量与筛选后口径 | 用户可能误解筛选结果 | 统一统计范围，或改为“匹配资产/匹配总值”并标注全量指标 |
| P2 | 移动端底部主导航没有直接地图入口 | 地图发现成本较高 | 在管理页或首页提供明确的“位置与地图”入口 |
| P2 | 首页同时完整展示最近添加和最近操作 | 小屏首屏信息密度偏高 | 合并为最近动态，或折叠次要区域 |
| P2 | 新增表单的可选字段较多 | 移动端录入路径较长 | 首屏保留最小字段，其余放入“更多信息” |
