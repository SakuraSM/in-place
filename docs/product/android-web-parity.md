...
下表表示功能入口覆盖；字段、筛选、失败恢复和设备验证的后续对齐情况见[优化批次与验收记录](optimization-batches.md)。B01 已统一导出上下文、多标签 OR 规则和批量部分失败恢复，并于 2026-09-12 完成 Android 35 原生验收，包括系统分享取文件和批量失败重试。其他批次仍按各自记录推进。
