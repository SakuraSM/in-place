...
export function isLocationItem(item: Pick<Item, 'type' | 'metadata'> | null | undefined): boolean {
  return Boolean(item?.type === 'container' && item.metadata?.[LOCATION_TAG_KEY] === true);
}
...
function resolveItemEstimatedValue(item: Item): number {
  if (item.type !== 'item' || item.price === null) return 0;
...
function buildSearchableText(item: Item, path: string[]): string {
  return [item.name, item.category, ...item.tags, ...path]
    .join(' ')
    .toLocaleLowerCase('zh-CN');
}

function sortAssetMapNodes(nodes: AssetMapNode[]): void {
  const kindOrder: Record<AssetMapNodeKind, number> = { location: 0, container: 1, item: 2 };
  nodes.sort((left, right) => left.kind === right.kind
    ? left.item.name.localeCompare(right.item.name, 'zh-CN')
    : kindOrder[left.kind] - kindOrder[right.kind]);
}

function calculateAggregateMetrics(nodeId: string, context: AggregateCalculationContext): AggregateMetrics {
  const cachedMetrics = context.aggregateCache.get(nodeId);
  if (cachedMetrics) return cachedMetrics;

  const node = context.nodesById.get(nodeId);
  if (!node || context.activeNodeIds.has(nodeId)) return EMPTY_AGGREGATE_METRICS;

  context.activeNodeIds.add(nodeId);
  ...
  for (const child of context.childrenByParentId.get(nodeId) ?? []) {
  ...
  context.activeNodeIds.delete(nodeId);
  context.aggregateCache.set(nodeId, aggregateMetrics);
...
export function buildAssetMapProjection(items: Item[]): AssetMapProjection {
  ...
  const totals: AssetMapTotals = { locations: 0, containers: 0, items: 0, totalQuantity: 0, estimatedValue: 0 };
  ...
  for (const item of items) {
    ...
    if (item.category.trim()) categoryNames.add(item.category.trim());

    if (kind === 'location') totals.locations += 1;
    else if (kind === 'container') totals.containers += 1;
    else {
      totals.items += 1;
  ...
  for (const siblings of childrenByParentId.values()) sortAssetMapNodes(siblings);

  const aggregateCache = new Map<string, AggregateMetrics>();
  for (const node of nodesById.values()) {
    const aggregateMetrics = calculateAggregateMetrics(node.id, {
      nodesById,
      childrenByParentId,
      aggregateCache,
